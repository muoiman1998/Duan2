...
import hieuboy.struts.dao.OrderDAOImpl;
import hieuboy.struts.dao.OrderDetailDAOImpl;
import hieuboy.struts.dao.UserDAOImpl;
import hieuboy.struts.entities.Order;
import hieuboy.struts.entities.OrderDetail;
import hieuboy.struts.entities.User;
import hieuboy.struts.utils.Cart;
...
public class CheckoutAction extends ActionSupport implements SessionAware {
	...
	public String view() {
		...
		if (cart == null || cart.getCount() == 0) {
			...
			session.put("error", "Vui lòng đăng nhập trước khi đặt mua! ");
			return ERROR;
		} else {
			// update
			...
			if (orderDAOImpl.insert(order)) {
				...
				session.remove("cart");
				session.put("success", " Đặt hàng thành công! Chúng tôi sẽ liên hệ với bạn sớm nhất.");
			}
		}
